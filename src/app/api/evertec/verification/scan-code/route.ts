/**
 * POST /api/evertec/verification/scan-code
 * Initiates barcode/QR code scan
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.SCAN_CODE,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/verification/scan-code',
    description: 'Initiates barcode/QR code scan',
    requestBody: {
      type: 'object',
    ...
    notes: ['Initiates barcode/QR code scan'],
  });
}
