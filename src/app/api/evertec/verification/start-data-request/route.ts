/**
 * POST /api/evertec/verification/start-data-request
 * Prompts customer for input data
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartDataRequestRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_DATA_REQUEST,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/verification/start-data-request',
    description: 'Prompts customer for input data',
    requestBody: {
      type: 'object',
    ...
    notes: ['Prompts customer for input data'],
  });
}
