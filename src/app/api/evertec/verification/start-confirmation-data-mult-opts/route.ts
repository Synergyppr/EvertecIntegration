/**
 * POST /api/evertec/verification/start-confirmation-data-mult-opts
 * Shows confirmation prompt with multiple options
 */
...
import type {
  StartConfirmationDataMultOptsRequest,
  TransactionResponse,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartConfirmationDataMultOptsRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_CONFIRMATION_DATA_MULT_OPTS,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/verification/start-confirmation-data-mult-opts',
    description: 'Shows confirmation prompt with multiple options',
    requestBody: {
      type: 'object',
    ...
    notes: ['Shows confirmation prompt with multiple options'],
  });
}
