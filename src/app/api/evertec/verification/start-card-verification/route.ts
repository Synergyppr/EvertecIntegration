/**
 * POST /api/evertec/verification/start-card-verification
 * Verifies card without charging
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartCardVerificationRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_CARD_VERIFICATION,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/verification/start-card-verification',
    description: 'Verifies card without charging',
    requestBody: {
      type: 'object',
    ...
    notes: ['Verifies card without charging'],
  });
}
