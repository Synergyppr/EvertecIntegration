/**
 * POST /api/evertec/verification/start-confirmation-data-2-opts
 * Shows confirmation prompt with 2 options
 */
...
import type {
  StartConfirmationData2OptsRequest,
  TransactionResponse,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartConfirmationData2OptsRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_CONFIRMATION_DATA_2_OPTS,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/verification/start-confirmation-data-2-opts',
    description: 'Shows confirmation prompt with 2 options',
    requestBody: {
      type: 'object',
    ...
    notes: ['Shows confirmation prompt with 2 options'],
  });
}
