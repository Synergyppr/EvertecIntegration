/**
 * POST /api/evertec/signature/get-signature-file
 * Retrieves signature image from last transaction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: GetSignatureFileRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.GET_SIGNATURE_FILE,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/signature/get-signature-file',
    description: 'Retrieves signature image from last transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Retrieves signature image from last transaction'],
  });
}
