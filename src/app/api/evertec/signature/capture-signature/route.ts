/**
 * POST /api/evertec/signature/capture-signature
 * Prompts customer to sign on terminal
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: CaptureSignatureRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.CAPTURE_SIGNATURE,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/signature/capture-signature',
    description: 'Prompts customer to sign on terminal',
    requestBody: {
      type: 'object',
    ...
    notes: ['Prompts customer to sign on terminal'],
  });
}
