/**
 * POST /api/evertec/display/items-list
 * Displays product list on terminal
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: ItemsListRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.ITEMS_LIST,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/display/items-list',
    description: 'Displays product list on terminal',
    requestBody: {
      type: 'object',
    ...
    notes: ['Displays product list on terminal'],
  });
}
