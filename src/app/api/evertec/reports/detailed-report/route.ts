/**
 * POST /api/evertec/reports/detailed-report
 * Generates detailed transaction report
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: DetailedReportRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.DETAILED_REPORT,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/reports/detailed-report',
    description: 'Generates detailed transaction report',
    requestBody: {
      type: 'object',
    ...
    notes: ['Generates detailed transaction report'],
  });
}
