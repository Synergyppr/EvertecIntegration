/**
 * POST /api/evertec/reports/totals-report
 * Generates totals summary report
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: TotalsReportRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.TOTALS_REPORT,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/reports/totals-report',
    description: 'Generates totals summary report',
    requestBody: {
      type: 'object',
    ...
    notes: ['Generates totals summary report'],
  });
}
