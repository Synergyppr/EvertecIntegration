/**
 * POST /api/evertec/cash/start-cash-refund
 * Processes a cash refund transaction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartCashRefundRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_CASH_REFUND,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/cash/start-cash-refund',
    description: 'Processes a cash refund transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes a cash refund transaction'],
  });
}
