/**
 * POST /api/evertec/cash/start-cash
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartCashRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_CASH,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/cash/start-cash',
    description: 'Processes a cash transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes a cash transaction'],
  });
}
