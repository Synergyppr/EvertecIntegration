 ...
 * Captures a pre-authorized transaction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.COMPLETION,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/preauth/completion',
    description: 'Captures a pre-authorized transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Captures a pre-authorized transaction'],
  });
}
