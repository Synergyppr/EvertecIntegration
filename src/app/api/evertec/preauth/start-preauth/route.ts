 ...
 * Reserves amount on card without capture
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartPreAuthRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_PRE_AUTH,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/preauth/start-preauth',
    description: 'Reserves amount on card without capture',
    requestBody: {
      type: 'object',
    ...
    notes: ['Reserves amount on card without capture'],
  });
}
