 ...
 * Processes an EBT Cash purchase with cashback
 */
...
import type {
  StartEbtCashPurchaseCashbackRequest,
  TransactionResponse,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartEbtCashPurchaseCashbackRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_EBT_CASH_PURCHASE_CASHBACK,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/ebt/cash-purchase-cashback',
    description: 'Processes an EBT Cash purchase with cashback',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes an EBT Cash purchase with cashback'],
  });
}
