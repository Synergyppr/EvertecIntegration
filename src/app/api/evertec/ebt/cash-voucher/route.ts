/**
 * POST /api/evertec/ebt/cash-voucher
 * Processes an EBT Cash voucher transaction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartEbtCashVoucherRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_EBT_CASH_VOUCHER,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/ebt/cash-voucher',
    description: 'Processes an EBT Cash voucher transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes an EBT Cash voucher transaction'],
  });
}
