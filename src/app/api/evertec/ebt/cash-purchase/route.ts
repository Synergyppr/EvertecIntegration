/**
 * POST /api/evertec/ebt/cash-purchase
 * Processes an EBT Cash purchase transaction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartEbtCashPurchaseRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_EBT_CASH_PURCHASE,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/ebt/cash-purchase',
    description: 'Processes an EBT Cash purchase transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes an EBT Cash purchase transaction'],
  });
}
