/**
 * POST /api/evertec/ebt/foodstamp-refund
 * Processes an EBT FoodStamp refund transaction
 */
...
import type {
  StartEbtFoodStampRefundRequest,
  TransactionResponse,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartEbtFoodStampRefundRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_EBT_FOODSTAMP_REFUND,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/ebt/foodstamp-refund',
    description: 'Processes an EBT FoodStamp refund transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes an EBT FoodStamp refund transaction'],
  });
}
