/**
 * POST /api/evertec/ebt/cash-withdrawal
 * Processes an EBT Cash withdrawal transaction
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartEbtCashWithdrawalRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_EBT_CASH_WITHDRAWAL,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/ebt/cash-withdrawal',
    description: 'Processes an EBT Cash withdrawal transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes an EBT Cash withdrawal transaction'],
  });
}
