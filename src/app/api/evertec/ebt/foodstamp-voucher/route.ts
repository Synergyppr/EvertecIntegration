/**
 * POST /api/evertec/ebt/foodstamp-voucher
 * Processes an EBT FoodStamp voucher transaction
 */
...
import type {
  StartEbtFoodStampVoucherRequest,
  TransactionResponse,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartEbtFoodStampVoucherRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_EBT_FOODSTAMP_VOUCHER,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/ebt/foodstamp-voucher',
    description: 'Processes an EBT FoodStamp voucher transaction',
    requestBody: {
      type: 'object',
    ...
    notes: ['Processes an EBT FoodStamp voucher transaction'],
  });
}
