...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartEbtBalanceInquiryRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_EBT_BALANCE_INQUIRY,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/ebt/balance-inquiry',
    description: 'Checks EBT card balance',
    requestBody: {
      type: 'object',
