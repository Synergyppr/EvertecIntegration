/**
 * POST /api/evertec/device/custom-print
 * Prints custom text on terminal
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: CustomPrintRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.CUSTOM_PRINT,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/device/custom-print',
    description: 'Prints custom text on terminal',
    requestBody: {
      type: 'object',
    ...
    notes: ['Prints custom text on terminal'],
  });
}
