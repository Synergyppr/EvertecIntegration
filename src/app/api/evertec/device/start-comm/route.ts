/**
 * POST /api/evertec/device/start-comm
 * Tests terminal communication
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: StartCommRequest = {
    ...
    const { data, status } = await makeTerminalRequest<TransactionResponse>(
      EVERTEC_ECR_ENDPOINTS.START_COMM,
      payload
    );
...
export async function GET() {
  return createApiDocumentation({
    endpoint: '/api/evertec/device/start-comm',
    description: 'Tests terminal communication',
    requestBody: {
      type: 'object',
    ...
    notes: ['Tests terminal communication'],
  });
}
